...
class Rewarder(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
        ...
        self.weight = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size])
        self.temperature = tf.compat.v1.placeholder(tf.float32, name='temperature')
        self.learning_rate = tf.compat.v1.placeholder(tf.float32, name="lr")
        ...
        with tf.device("/cpu:0"):
            self.word = tf.nn.dropout(tf.nn.embedding_lookup(params=self.r_embeddings, ids=self.x), 1 - (self.dropout_keep_prob))
            self.processed_x = tf.transpose(a=self.word, perm=[1, 0, 2])  # seq_length x batch_size x emb_dim
        ...
        _, _, _, self.r_predictions, self.gen_h = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(params=self.r_embeddings, ids=self.start_token),
                       self.h0, r_predictions, gen_h))

        self.r_predictions = tf.transpose(a=self.r_predictions.stack(), perm=[1, 0, 2])  # batch_size x seq_length x vocab_size

        # clip_reward & log_pred :  batch*seq  x vocab_size
        self.clipped_reward = tf.one_hot(tf.cast(tf.reshape(self.x, [-1]), dtype=tf.int32), self.num_emb, 1.0, 0.0) * \
            tf.clip_by_value(tf.reshape(self.r_predictions, [-1, self.num_emb]), 1e-20, 1.0)
        self.reward_per_step_snt = tf.reshape(tf.reduce_sum(input_tensor=self.clipped_reward, axis=-1), [self.batch_size, self.sequence_length])
        self.sent_reward = tf.reduce_sum(input_tensor=self.reward_per_step_snt, axis=1)
        ent_w= 1
        save_dir="./save"
#         save_dir = os.path.abspath(save_dir)

        eval_file_prefix = os.path.join(save_dir,'evaler_file'+str(ent_w))
        with open(eval_file_prefix + "_dloss.txt","r",encoding="utf-8") as f:
            d_loss_supervised = f.read()
        self.reward_loss = -tf.reduce_sum(input_tensor=self.sent_reward * self.f_weight) + \
                            l2_reg_lambda * (tf.add_n([tf.nn.l2_loss(var) for var in self.r_params if var not in [self.r_embeddings]])) + float(d_loss_supervised)

        reward_opt = self.optimizer(self.learning_rate)

        self.reward_grad, _ = tf.clip_by_global_norm(tf.gradients(ys=self.reward_loss, xs=self.r_params), self.grad_clip)
    ...
    def create_output_unit(self, params, midlayer, num_highway=2):
        ...
        def unit(hidden_memory_tuple):
            ...
            hidden_state = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(hidden_state, self.Wbo_list[0], self.Wbo_list[1]))
            for i in range(num_highway):
                tran = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(hidden_state, self.Wbo_list[2 + 4 * i], self.Wbo_list[3 + 4 * i]))
                gate = tf.nn.sigmoid(tf.compat.v1.nn.xw_plus_b(hidden_state, self.Wbo_list[4 + 4 * i], self.Wbo_list[5 + 4 * i]))
                hidden_state = tran * gate + (1. - gate) * hidden_state

            rewards = tf.nn.sigmoid(tf.compat.v1.nn.xw_plus_b(hidden_state, self.Wbo_list[-2], self.Wbo_list[-1]))
            return rewards
