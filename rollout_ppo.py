...
class ROLLOUT(object):
    def __init__(self, lstm, update_rate, reward_gamma):
        ...
        # self.Wi = tf.range(self.Wi, [self.Wi[1,:10]],message='rollout.wi======',summarize=100)
        ...
        self.given_num = tf.compat.v1.placeholder(tf.int32)
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            log_prob = tf.math.log(tf.nn.softmax(o_t))
            next_token = tf.cast(tf.reshape(tf.random.categorical(logits=log_prob, num_samples=1), [self.batch_size]), tf.int32)
            clipped_log_prob = tf.math.log(tf.clip_by_value(tf.nn.softmax(o_t), 1e-20, 1.0))
            x_tp1 = tf.nn.embedding_lookup(params=self.g_embeddings, ids=next_token)  # batch x emb_dim
            gen_o = gen_o.write(i, tf.reduce_sum(input_tensor=tf.multiply(tf.one_hot(next_token, self.num_emb, 1.0, 0.0),
                                                             clipped_log_prob), axis=1))  # [batch_size] , prob
            gen_x = gen_x.write(i, next_token)  # indices, batch_size
        ...
        _, _, _, self.gen_o_old, self.gen_x_old = control_flow_ops.while_loop(
            ...
            body=_g_recurrence,
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(params=self.g_embeddings, ids=self.start_token), self.h0, gen_o_old, gen_x_old))
        ...
        self.gen_x_old = tf.transpose(a=self.gen_x_old, perm=[1, 0])

        self.gen_o_old = tf.transpose(a=self.gen_o_old.stack(), perm=[1, 0])
    ...
    def create_recurrent_unit(self):
        ...
        # self.Wi = tf.range(self.Wi, [self.Wi[1,:10]],message='self.wi======',summarize=100)
