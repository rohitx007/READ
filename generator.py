...
class Generator(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
        ...
        self.off_policy_prob = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size, self.sequence_length], name='off_policy_prob')
        self.baseline = tf.compat.v1.placeholder(tf.float32, shape=[self.sequence_length], name='baseline')
        self.rewards = tf.compat.v1.placeholder(tf.float32, shape=[self.batch_size, self.sequence_length], name='rewards')
        self.decay_weight = tf.compat.v1.placeholder(tf.float32, name="decay_weight")

        # processed for batch
        # self.f_weight = tf.Print(self.f_weight, [self.f_weight[-3:]], message='='*10)
        with tf.device("/cpu:0"):
            self.word = tf.nn.embedding_lookup(params=self.g_embeddings, ids=self.x)
        ...
        self.clipped_log_pred = tf.one_hot(tf.cast(tf.reshape(self.x, [-1]), dtype=tf.int32), self.num_emb, 1.0, 0.0) * tf.math.log(
            tf.clip_by_value(tf.reshape(self.g_predictions, [-1, self.num_emb]), 1e-20, 1.0))

        self.sent_log = tf.reduce_sum(input_tensor=tf.reshape(tf.reduce_sum(input_tensor=self.clipped_log_pred, axis=-1), [self.batch_size, self.sequence_length]), axis=1)
        # pretraining loss
        self.pretrain_loss = -tf.reduce_sum(input_tensor=self.clipped_log_pred) / (self.sequence_length * self.batch_size)
        ...
        #######################################################################################################
        log_pred = tf.reduce_sum(input_tensor=self.clipped_log_pred, axis=-1)
        ...
        self.g_loss = - tf.reduce_mean(input_tensor=tf.minimum(choice_a, choice_b))
    ...
    def create_output_unit(self, params, midlayer):
        ...
        def unit(hidden_memory_tuple):
            ...
            for j in range(len(self.Wbo_list) // 2 - 1):
                hidden_state = tf.nn.relu(tf.compat.v1.nn.xw_plus_b(hidden_state, self.Wbo_list[2 * j], self.Wbo_list[2 * j + 1]))
            logits = tf.compat.v1.nn.xw_plus_b(hidden_state, self.Wbo_list[-2], self.Wbo_list[-1])
